...
**API layer** — two parallel styles coexist, prefer the class-based one for new code:
...
- `lib/api-models/*` (e.g. `ProductAPI`, `UserAPI`) wrap `APIHandler` in per-resource classes — this is the preferred pattern for new API code, exposed via fixtures as `productApi`/`userApi`.
- `lib/utils/api-utils.ts` has older free-function equivalents (`createProductAPI`, `getUserByEmailAPI`, etc.) kept for existing tests; avoid adding new functions here in favor of the `api-models` classes.

**Fixtures** (`lib/fixtures/*`, each exports its own `test`, layered via `.extend`): `apiFixtures` is the base (`apiHandler`, `adminToken`, `productApi`, `userApi`, plus worker-scoped `apiHandlerWorker`/`adminTokenWorker`/`userApiWorker`/`baseAPIUrl`). `catalogueFixtures` adds `cataloguePage`. `dbFixtures` adds direct MySQL query fixtures (`getNewUserId`). `getAuthenticatedUser`/`getAuthenticatedUserGlobal`/`adminStorageState` build/refresh `storageState` JSON files under `playwright/.auth/` so specs can skip UI login (a test/worker-scoped user is registered via API and its token is written directly into a storage-state file consumed by `page`). Specs import `test` from whichever fixtures file provides what they need (`@fixtures/apiFixtures`, `@fixtures/adminStorageState`, etc.) — never from `@playwright/test` directly. `global.setup.ts` and `tests/auth.setup.ts` are legacy/experimental global-auth approaches not currently wired into `playwright.config.ts` (the `setup`/`teardown` projects are commented out) — the active pattern is the per-fixture storage-state files above.
